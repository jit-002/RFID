...
export const DancingLetters: React.FC<DancingLettersProps> = ({
  ...
  return (
    <LazyMotion features={domAnimation}>
      <m.div
        className={cn(
          "flex flex-wrap items-center justify-center select-none gap-x-0.5",
          className
        )}
      ...
      >
        {letters.map((letter, id) => {
          if (letter === " ") {
            return <span key={`space-${id}`} className="inline-block w-3 md:w-5" />;
          }
          ...
          return (
            <m.span
              ...
              className={cn(
                "relative inline-block font-bold cursor-pointer transition-colors duration-200",
                letterClassName,
                isActive ? "z-10 text-cyan-400" : "z-0"
              )}
              style={{
